...
export function DonationAnalyticsCharts({
  filters,
}: DonationAnalyticsChartsProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const response =
          await mockAnalyticsService.getDonationAnalytics(filters);
    ...
    return (
      <Card withBorder radius="md" p="lg">
        <Text c="dimmed" ta="center">
          Failed to load donation analytics data
        </Text>
      </Card>
    );
  }
  ...
  const topPurposesData = data.topDonationPurposes.map(item => ({
    purpose:
      item.purpose.length > 15
        ? item.purpose.substring(0, 15) + '...'
        : item.purpose,
    amount: item.amount,
    count: item.count,
    Amount: item.amount,
  ...
  const donorSegmentData = data.donorSegments.map(item => ({
    ...
    count: item.count,
    color: item.segment.includes('Major')
      ? 'var(--mantine-color-green-6)'
      : item.segment.includes('Regular')
      ...
      <Card withBorder radius="md" p="lg">
        ...
        <Group justify="space-between" mb="xs">
          ...
          <Badge
            size="sm"
            variant="light"
            color={data.donorRetentionRate > 50 ? 'green' : 'orange'}
          >
        ...
        <Progress
          ...
          size="sm"
          color={
            data.donorRetentionRate > 50
              ? 'green'
              : data.donorRetentionRate > 30
                ? 'yellow'
                : 'red'
          }
          mt="md"
