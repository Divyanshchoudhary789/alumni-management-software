...
import { Card, Title, Stack, Text, Group, Avatar, Badge, Button, Skeleton, ThemeIcon } from '@mantine/core';
...
export function DonorRecognition({ stats, loading }: DonorRecognitionProps) {
  if (loading) {
    return (
      <Card padding="lg" radius="md" withBorder>
        <Title order={3} mb="md">Top Donors</Title>
        <Stack gap="md">
          {[...Array(5)].map((_, index) => (
  ...
  const getRecognitionLevel = (amount: number, rank: number) => {
    if (rank === 0) return { level: 'Platinum', color: 'violet', icon: IconTrophy };
    if (rank === 1) return { level: 'Gold', color: 'yellow', icon: IconMedal };
    if (rank === 2) return { level: 'Silver', color: 'gray', icon: IconAward };
    if (amount >= 5000) return { level: 'Bronze', color: 'orange', icon: IconStar };
    if (amount >= 1000) return { level: 'Supporter', color: 'blue', icon: IconStar };
    return { level: 'Friend', color: 'green', icon: IconStar };
  };

  const getRankIcon = (rank: number) => {
    switch (rank) {
      case 0: return <IconTrophy size={16} color="gold" />;
      case 1: return <IconMedal size={16} color="silver" />;
      case 2: return <IconAward size={16} color="#CD7F32" />;
      default: return <Text size="sm" fw={500} c="dimmed">#{rank + 1}</Text>;
    }
  };
  ...
  return (
    <Card padding="lg" radius="md" withBorder>
      ...
      <Card mt="md" padding="sm" radius="sm" bg="gray.0">
        <Text size="xs" c="dimmed" mb="xs">Recognition Levels</Text>
