...
export const generateMockDonations = (count: number = 100, alumniIds: string[]): Donation[] => {
  const purposes = [
    'General Fund', 'Scholarship Fund', 'Alumni Events', 'Infrastructure Development',
    'Research Fund', 'Student Support', 'Faculty Development', 'Technology Upgrade',
    'Library Fund', 'Athletic Programs'
  ];
  
  const paymentMethods = ['Credit Card', 'Bank Transfer', 'PayPal', 'Check', 'Cryptocurrency'];
  ...
  for (let i = 6; i <= count + 5; i++) {
    ...
    const donationDate = new Date(2023, 2 + Math.floor(Math.random() * 12), Math.floor(Math.random() * 28) + 1);
