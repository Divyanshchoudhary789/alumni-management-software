...
class MockAlumniService {
  ...
  async getAlumni(
    ...
    if (simulateError(0.02)) { // 2% error rate
    ...
    if (filters) {
      if (filters.search) {
        filteredAlumni = filterByText(filteredAlumni, filters.search, [
          'firstName', 'lastName', 'currentCompany', 'currentPosition', 'degree'
        ]);
      }
      ...
      if (filters.isPublic !== undefined) {
        filteredAlumni = filteredAlumni.filter(a => a.isPublic === filters.isPublic);
      }
    }
  ...
  async createAlumni(alumniData: Omit<AlumniProfile, 'id' | 'createdAt' | 'updatedAt'>): Promise<MockApiResponse<AlumniProfile>> {
    ...
    if (!alumniData.firstName || !alumniData.lastName || !alumniData.graduationYear) {
    ...
    alumniData = [...alumniData, newAlumni];
    
    return createSuccessResponse(newAlumni, 'Alumni profile created successfully');
  }
  
  // Update alumni profile
  async updateAlumni(id: string, updates: Partial<AlumniProfile>): Promise<MockApiResponse<AlumniProfile>> {
    ...
    return createSuccessResponse(updatedAlumni, 'Alumni profile updated successfully');
  }
  ...
  async getAlumniStats(): Promise<MockApiResponse<{
    totalAlumni: number;
    ...
    // Top companies
    const companyCount = alumniData.reduce((acc, alumni) => {
    ...
    // Top skills
    const skillCount = alumniData.reduce((acc, alumni) => {
    ...
    const locationDistribution = alumniData.reduce((acc, alumni) => {
      if (alumni.location) {
  ...
  async searchAlumni(query: string, options?: {
    ...
    if (query) {
      searchResults = filterByText(searchResults, query, [
        'firstName', 'lastName', 'currentCompany', 'currentPosition', 
        'degree', 'location', 'skills', 'interests'
      ]);
    }
