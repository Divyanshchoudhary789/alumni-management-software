...
class MockDashboardService {
  ...
  async getDashboardMetrics(): Promise<MockApiResponse<DashboardMetrics>> {
    ...
    const metrics: DashboardMetrics = {
      ...
      upcomingEvents: mockData.events.filter(e => 
        e.status === 'published' && e.eventDate > new Date()
      ).length,
  ...
  async getRecentActivities(limit: number = 10): Promise<MockApiResponse<RecentActivity[]>> {
  ...
  async getAlumniGrowthData(period: 'month' | 'quarter' | 'year' = 'month'): Promise<MockApiResponse<Array<{ period: string; count: number }>>> {
  ...
  async getDonationTrends(period: 'month' | 'quarter' | 'year' = 'month'): Promise<MockApiResponse<Array<{ period: string; amount: number; count: number }>>> {
    await simulateDelay(150, 300);
  ...
  async getEventAttendanceData(): Promise<MockApiResponse<Array<{ eventName: string; registered: number; attended: number; capacity: number }>>> {
    ...
    const completedEvents = mockData.events
      ...
      .slice(0, 10)
      .map(event => {
        const registrations = mockData.eventRegistrations.filter(r => r.eventId === event.id);
  ...
  async getSummaryStats(): Promise<MockApiResponse<{
    totalAlumni: number;
    ...
    const stats = {
      totalAlumni: mockData.alumni.length,
      totalDonations: mockData.donations.filter(d => d.status === 'completed').length,
      totalEvents: mockData.events.length,
      totalCommunications: mockData.communications.filter(c => c.status === 'sent').length,
      growthRates: {
