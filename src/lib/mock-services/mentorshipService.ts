...
const mentorshipData = [...mockData.mentorshipConnections];
const mentorProfilesData = [...mockMentorProfiles];
const menteeRequestsData = [...mockMenteeRequests];
...
class MockMentorshipService {
  ...
  async getMentorshipConnections(
    ...
    if (filters) {
      if (filters.search) {
        filteredConnections = filterByText(
          filteredConnections,
          filters.search,
          ['notes']
        );
      ...
      if (filters.mentorId) {
        filteredConnections = filteredConnections.filter(
          c => c.mentorId === filters.mentorId
        );
      }

      if (filters.menteeId) {
        filteredConnections = filteredConnections.filter(
          c => c.menteeId === filters.menteeId
        );
      }

      if (filters.dateFrom) {
        filteredConnections = filteredConnections.filter(
          c => c.startDate >= filters.dateFrom!
        );
      }

      if (filters.dateTo) {
        filteredConnections = filteredConnections.filter(
          c => c.startDate <= filters.dateTo!
        );
      }
    }
    ...
    // Apply sorting
    if (sort) {
      filteredConnections = sortByField(
        filteredConnections,
        sort.field,
        sort.direction
      );
  ...
  async createMentorshipConnection(
    ...
    const existingConnection = mentorshipData.find(
      c =>
        c.mentorId === connectionData.mentorId &&
    ...
    if (existingConnection) {
      throw createErrorResponse({
        code: 'VALIDATION_ERROR',
        message:
          'An active or pending mentorship connection already exists between these users.',
      });
    }

    // Check mentor availability
    const mentorProfile = mentorProfilesData.find(
      p => p.alumniId === connectionData.mentorId
    );
    if (mentorProfile) {
    ...
    if (mentorProfile && newConnection.status === 'active') {
      const profileIndex = mentorProfilesData.findIndex(
        p => p.alumniId === connectionData.mentorId
      );
      if (profileIndex !== -1) {
    ...
    return createSuccessResponse(
      newConnection,
      'Mentorship connection created successfully'
    );
  }
  ...
  async updateMentorshipConnection(
    ...
    if (updates.status && updates.status !== existingConnection.status) {
      const mentorProfile = mentorProfilesData.find(
        p => p.alumniId === existingConnection.mentorId
      );
      if (mentorProfile) {
        const profileIndex = mentorProfilesData.findIndex(
          p => p.alumniId === existingConnection.mentorId
    ...
    return createSuccessResponse(
      updatedConnection,
      'Mentorship connection updated successfully'
    );
  }

  // Delete mentorship connection
  async deleteMentorshipConnection(
    id: string
  ): Promise<MockApiResponse<{ id: string }>> {
    ...
    if (connection.status === 'active') {
      const mentorProfile = mentorProfilesData.find(
        p => p.alumniId === connection.mentorId
      );
      if (mentorProfile) {
        const profileIndex = mentorProfilesData.findIndex(
          p => p.alumniId === connection.mentorId
    ...
    return createSuccessResponse(
      { id },
      'Mentorship connection deleted successfully'
    );
  }
  ...
  async getMentorProfiles(
    available?: boolean
  ): Promise<MockApiResponse<typeof mentorProfilesData>> {
    ...
    if (available !== undefined) {
      profiles = profiles.filter(p =>
        available
          ? p.isActive && p.currentMentees < p.maxMentees
          : !p.isActive || p.currentMentees >= p.maxMentees
      );
    }
  ...
  async getMenteeRequests(
    status?: string
  ): Promise<MockApiResponse<typeof menteeRequestsData>> {
    await simulateDelay(100, 250);
  ...
  async createMenteeRequest(
    requestData: Omit<
      (typeof menteeRequestsData)[0],
      'id' | 'createdAt' | 'updatedAt' | 'status'
    >
  ): Promise<MockApiResponse<(typeof menteeRequestsData)[0]>> {
    await simulateDelay(200, 400);
  ...
  async suggestMentorMatches(menteeRequestId: string): Promise<
    MockApiResponse<
      Array<{
        mentorId: string;
        score: number;
  ...
  > {
    ...
    const menteeRequest = menteeRequestsData.find(
      r => r.id === menteeRequestId
    );
    ...
    const availableMentors = mentorProfilesData.filter(
  ...
  > {
    ...
    const activeConnections = mentorshipData.filter(
      c => c.status === 'active'
    ).length;
    const completedConnections = mentorshipData.filter(
      c => c.status === 'completed'
    ).length;
    const pendingConnections = mentorshipData.filter(
      c => c.status === 'pending'
    ).length;
    ...
    const pendingRequests = menteeRequestsData.filter(
      r => r.status === 'pending'
    ).length;

    const successRate =
      totalConnections > 0
        ? (completedConnections / totalConnections) * 100
        : 0;

    // Calculate average duration for completed mentorships
    const completedWithDuration = mentorshipData.filter(
      c => c.status === 'completed'
    );
