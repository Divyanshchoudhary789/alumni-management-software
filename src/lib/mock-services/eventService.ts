...
const eventsData = [...mockData.events];
...
class MockEventService {
  ...
  async getEvents(
    ...
    if (filters) {
      ...
      if (filters.dateFrom) {
        filteredEvents = filteredEvents.filter(
          e => e.eventDate >= filters.dateFrom!
        );
      }

      if (filters.dateTo) {
        filteredEvents = filteredEvents.filter(
          e => e.eventDate <= filters.dateTo!
        );
      }
  ...
  // Create new event
  async createEvent(
    eventData: Omit<Event, 'id' | 'createdAt' | 'updatedAt' | 'registrations'>
  ): Promise<MockApiResponse<Event>> {
  ...
  async registerForEvent(
    eventId: string,
    alumniId: string
  ): Promise<MockApiResponse<EventRegistration>> {
    await simulateDelay(200, 400);
  ...
  > {
    ...
    const upcomingEvents = eventsData.filter(
      e => e.status === 'published' && e.eventDate > now
    ).length;
    const completedEvents = eventsData.filter(
      e => e.status === 'completed'
    ).length;
    const totalRegistrations = registrationsData.filter(
    ...
    const completedEventsWithAttendance = eventsData
      ...
      .map(event => {
        const registrations = registrationsData.filter(
    ...
    const averageAttendance =
      completedEventsWithAttendance.length > 0
        ? completedEventsWithAttendance.reduce((sum, rate) => sum + rate, 0) /
    ...
    const popularEvents = eventsData
      .map(event => ({
        eventId: event.id,
        title: event.title,
        registrations: registrationsData.filter(
