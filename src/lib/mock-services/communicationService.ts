...
const communicationsData = [...mockData.communications];
const templatesData = [...mockCommunicationTemplates];
...
class MockCommunicationService {
  ...
  async getCommunications(
    ...
    if (filters) {
      if (filters.search) {
        filteredCommunications = filterByText(
      ...
      if (filters.dateFrom) {
        filteredCommunications = filteredCommunications.filter(
          c => c.sentDate && c.sentDate >= filters.dateFrom!
        );
      }

      if (filters.dateTo) {
        filteredCommunications = filteredCommunications.filter(
          c => c.sentDate && c.sentDate <= filters.dateTo!
        );
      }
    }
    ...
    // Apply sorting
    if (sort) {
      filteredCommunications = sortByField(
        filteredCommunications,
        sort.field,
        sort.direction
      );
  ...
  async createCommunication(
    communicationData: Omit<
      Communication,
      'id' | 'createdAt' | 'updatedAt' | 'sentDate' | 'status'
    >
  ): Promise<MockApiResponse<Communication>> {
    ...
    if (
      !communicationData.targetAudience ||
      communicationData.targetAudience.length === 0
    ) {
      throw createErrorResponse({
        code: 'VALIDATION_ERROR',
  ...
  async createFromTemplate(
    ...
    return createSuccessResponse(
      newCommunication,
      'Communication created from template'
    );
  }
  ...
  > {
    ...
    const scheduled = communicationsData.filter(
    ...
    // Type breakdown
    const typeBreakdown = communicationsData.reduce(
      (acc, comm) => {
