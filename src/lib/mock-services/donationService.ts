...
let donationsData = [...mockData.donations];
let campaignsData = [...mockDonationCampaigns];
...
class MockDonationService {
  ...
  async getDonations(
    ...
    if (filters) {
      ...
      if (filters.campaignId) {
        filteredDonations = filteredDonations.filter(d => d.campaignId === filters.campaignId);
      }
      
      if (filters.dateFrom) {
        filteredDonations = filteredDonations.filter(d => d.donationDate >= filters.dateFrom!);
      }
      
      if (filters.dateTo) {
        filteredDonations = filteredDonations.filter(d => d.donationDate <= filters.dateTo!);
      }
      
      if (filters.amountMin !== undefined) {
        filteredDonations = filteredDonations.filter(d => d.amount >= filters.amountMin!);
      }
      
      if (filters.amountMax !== undefined) {
        filteredDonations = filteredDonations.filter(d => d.amount <= filters.amountMax!);
      }
    }
    
    // Apply sorting
    if (sort) {
      filteredDonations = sortByField(filteredDonations, sort.field, sort.direction);
    } else {
      // Default sort by donation date (newest first)
      filteredDonations = sortByField(filteredDonations, 'donationDate', 'desc');
    }
  ...
  // Create new donation
  async createDonation(donationData: Omit<Donation, 'id' | 'createdAt' | 'updatedAt'>): Promise<MockApiResponse<Donation>> {
    await simulateDelay(300, 800); // Longer delay for payment processing
    
    if (simulateError(0.05)) { // Higher error rate for payment processing
    ...
    if (!donationData.donorId || !donationData.amount || !donationData.purpose) {
      throw createErrorResponse({
    ...
    if (donationData.amount > 100000) {
      throw createErrorResponse({
        code: 'VALIDATION_ERROR',
        message: 'Donation amount cannot exceed $100,000. Please contact us for larger donations.'
      });
    }
    ...
    setTimeout(() => {
      const donationIndex = donationsData.findIndex(d => d.id === newDonation.id);
      if (donationIndex !== -1) {
    ...
    return createSuccessResponse(newDonation, 'Donation submitted for processing');
  }
  
  // Update donation status (admin only)
  async updateDonationStatus(id: string, status: Donation['status']): Promise<MockApiResponse<Donation>> {
    ...
    return createSuccessResponse(updatedDonation, 'Donation status updated successfully');
  }
  ...
  async getDonationStats(): Promise<MockApiResponse<{
    ...
    topDonors: Array<{ donorId: string; totalAmount: number; donationCount: number }>;
    ...
    // Monthly totals
    const monthlyTotals = completedDonations.reduce((acc, donation) => {
    ...
    // Top donors
    const donorTotals = completedDonations.reduce((acc, donation) => {
    ...
    // Purpose breakdown
    const purposeBreakdown = completedDonations.reduce((acc, donation) => {
    ...
    const paymentMethodBreakdown = completedDonations.reduce((acc, donation) => {
  ...
  async getRecentDonations(limit: number = 10): Promise<MockApiResponse<Donation[]>> {
  ...
  async processPendingDonations(): Promise<MockApiResponse<{ processed: number; failed: number }>> {
